...
(function(){
  ...
  window.eqBaseStatsWithRaw=function(eq){
    ...
    const add=(key,label,value)=>{
      if(value!==undefined&&value!==null&&value!==''&&base[key]===undefined)base[key]={label,value:Number(value)||0};
    };
    const addRange=(key,label,min,max)=>{
      if((min!==undefined||max!==undefined)&&base[key]===undefined)base[key]={label,min:Number(min)||0,max:Number(max??min)||0};
    };
    add('m_attack','術攻',raw.MagicAttack);
    ...
    add('level','等級',raw.Level ?? eq?.base_stats?.level);
  ...
  window.renderEquipmentRandomPage=function(keepScroll=false){
    ...
    byId('reader').innerHTML=`<section class="card"><button class="backBtn" id="eqBackToSim">← 返回裝備合成模擬</button><h1>合成亂數模擬</h1><h2>${esc(eq.name)}</h2>${eqMetaLine(eq)}${eqRenderStats(eq,false)}${eqRenderRandomRecipePicker()}<div class="quick"><button type="button" class="primary" id="eqSimOnce">模擬一次<small>隨機產生本次合成增加值，並依配方累計材料</small></button><button type="button" id="eqSimClear" class="ghost">清空重新計算<small>歸零累計次數與材料，保留目前選擇的配方</small></button></div><div class="kvGrid"><div class="kv"><div class="k">累計模擬次數</div><div class="v">${fmt(eqState.simCount||0)}</div></div></div><h3>本次模擬紀錄</h3><div class="tableWrap"><table><thead><tr><th>次數</th><th>配方</th><th>本次增加</th></tr></thead><tbody>${histRows}</tbody></table></div><h3>依模擬次數累計材料</h3>${materialSections}</section>`;
