// V222: Jiangshen main compare + 0~20 star table only.
// Support simulation is handled only by support-slots-compare.js.
(function(){
  ...
  function intAbility(a){ const out = {}; for(const st of stats()) out[st] = Math.ceil(Number(a?.[st] || 0)); return out; }
  ...
  function renderCompare(){
    ...
    r.innerHTML = `<section class="card"><h1>降神、經驗、修練試算</h1><h2>主降神比較</h2><div class="kvGrid"><div class="kv"><div class="k">降神 A</div><div class="v"><select id="jsA">${optionsNames()}</select><label>星等</label><select id="jsAS">${starOptions(20)}</select></div></div><div class="kv"><div class="k">降神 B</div><div class="v"><select id="jsB">${optionsNames()}</select><label>星等</label><select id="jsBS">${starOptions(20)}</select></div></div></div><div class="quick"><button id="calcCompare" type="button">產生比較頁<small>比較兩位主降神能力差異</small></button></div></section>`;
  }
  function renderStars(){
    const r = $('reader'); if(!r) return;
    r.innerHTML = `<section class="card"><h1>降神、經驗、修練試算</h1><h2>20星等</h2><div class="kvGrid"><div class="kv"><div class="k">選擇降神</div><div class="v"><select id="jsStarName">${optionsNames(false)}</select></div></div></div><div class="quick"><button id="calcStars" type="button">產生 0～20 星能力總表<small>完整顯示各星等能力</small></button></div></section>`;
  }
  ...
  window.calcCompare = function(){
    ...
    if(!a || !b){ if(typeof empty === 'function') empty('請選擇兩位降神'); else alert('請選擇兩位降神'); return; }
    const as = Number($('jsAS')?.value || 20), bs = Number($('jsBS')?.value || 20);
    const A = intAbility(getAbility(a, as)), B = intAbility(getAbility(b, bs));
    const r = $('reader'); if(!r) return;
    r.innerHTML = `<section class="card">${backBtn()}<h1>主降神比較</h1><div class="tableWrap"><table><thead><tr><th>能力</th><th>${E(a)}</th><th>${E(b)}</th><th>差異</th></tr></thead><tbody>${stats().map(st=>`<tr><td>${E(st)}</td><td>${fmt(A[st])}</td><td>${fmt(B[st])}</td><td>${fmt((A[st]||0)-(B[st]||0))}</td></tr>`).join('')}</tbody></table></div></section>`;
    scrollTop();
  };
  window.calcStars = function(){
    ...
    const r = $('reader'); if(!r) return;
    r.innerHTML = `<section class="card">${backBtn()}<h1>${E(n)}｜0～20 星能力總表</h1><div class="tableWrap"><table><thead><tr><th>星等</th>${stats().map(st=>`<th>${E(st)}</th>`).join('')}</tr></thead><tbody>${Array.from({length:21},(_,lv)=>{ const a=intAbility(getAbility(n,lv)); return `<tr><td>${lv} 星</td>${stats().map(st=>`<td>${fmt(a[st])}</td>`).join('')}</tr>`; }).join('')}</tbody></table></div></section>`;
    scrollTop();
  };
