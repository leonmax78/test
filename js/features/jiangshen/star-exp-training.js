...
(function(){
 ...
 const maxKey=t=>Math.max(0,...Object.keys(t||{}).map(Number).filter(Number.isFinite));
 const rowTotal=r=>Number(r?.total??r?.cum??0)||0;
 const expAt=lv=>{try{const t=data().expTable||{};return BigInt(t[String(lv)]||0)}catch(e){return 0n}};
 const maxLv=()=>{try{return maxKey(data().expTable)||2000}catch(e){return 2000}};
 const starCost=(a,b)=>{const t=data().starSoulTable||data().starRequirements||{};const max=maxKey(t)||20;const cur=Math.max(0,Math.min(max,Math.floor(Number(a)||0))),tar=Math.max(0,Math.min(max,Math.floor(Number(b)||0)));return tar>cur?Math.max(0,rowTotal(t[String(tar)])-rowTotal(t[String(cur)])):0};
 const auraCost=(a,b)=>{const t=data().auraTable||{};const max=maxKey(t)||400;const cur=Math.max(0,Math.min(max,Math.floor(Number(a)||0))),tar=Math.max(cur,Math.min(max,Math.floor(Number(b)||cur)));return Math.max(0,rowTotal(t[String(tar)])-rowTotal(t[String(cur)]))};
...
window.calcNeeds=function(){const cur=Math.max(0,Math.min(20,Math.floor(num('needCur',0)))),tar=Math.max(0,Math.min(20,Math.floor(num('needTar',20)))),owned=Math.max(0,Math.floor(num('needOwned',0))),rate=Math.max(.0001,num('needRate',1));const raw=tar>cur?starCost(cur,tar):0,after=Math.max(0,raw-owned),need=Math.ceil(after/rate);const b=$('starNeedResult');if(b)b.innerHTML=`<div class="kvGrid" style="margin-top:12px"><div class="kv"><div class="k">星等區間</div><div class="v">${cur} 星 → ${tar} 星</div></div><div class="kv"><div class="k">原始需求</div><div class="v">${fmt(raw)}</div></div><div class="kv"><div class="k">已有數量</div><div class="v">${fmt(owned)}</div></div><div class="kv"><div class="k">扣除已有後</div><div class="v">${fmt(after)}</div></div><div class="kv"><div class="k">倍率後還需</div><div class="v">${fmt(need)}</div></div></div>`};
 window.calcStarAura=function(){const cur=Math.max(0,Math.floor(num('auraCur',0))),tar=Math.max(cur,Math.floor(num('auraTar',cur)));const total=auraCost(cur,tar);const b=$('auraNeedResult');if(b)b.innerHTML=`<div class="kvGrid" style="margin-top:12px"><div class="kv"><div class="k">等級區間</div><div class="v">${cur} → ${tar}</div></div><div class="kv"><div class="k">需要靈氣</div><div class="v">${fmt(total)}</div></div></div>`};
